...
FR_INDEX = REPO / "docs" / "founder-requests" / "INDEX.md"
...
def inline_md(raw: str) -> str:
    """Escape, then re-admit the three inline markdown forms these source files use.

    The source documents are markdown; escaping them wholesale rendered literal
    `**` and backticks on the page, which read as corruption rather than emphasis.
    Escaping first and re-admitting a fixed three patterns keeps it injection-safe:
    nothing from the source can introduce a tag that is not one of these.
    """
    out = html.escape(raw)
...
def founder_requests(
    ...
    """Status tallies and the newest entries from the founder-request index.

    Returns (counts, newest, all_rows); newest and all_rows are (id, status, subject). The index is
    generated by tools/founder_requests.py, so this reads the same rows that tool
    writes rather than re-scanning the directory.
    """
    # Status is a section heading ("## NEW — 22"), not a column; rows beneath it
    # are "| [FR-018](file.md) | Subject | Raised | Source |".
    rows: list[tuple[str, str, str]] = []
    for status, block in re.findall(
        r"^##\s+([A-Z][A-Z \-]*?)\s+—.*?$(.*?)(?=^##\s|\Z)", index_text, re.S | re.M
    ):
        for fr_id, subject in re.findall(
            r"^\|\s*\[(FR-\d+)\]\([^)]*\)\s*\|\s*(.+?)\s*\|", block, re.M
        ):
            rows.append((fr_id, status, subject))
    if not rows:
        return [], [], []
    tally: dict[str, int] = {}
    for _, status, _subject in rows:
        tally[status] = tally.get(status, 0) + 1
    order = ["NEW", "SCOPING", "SPECCED", "IN PROGRESS", "SHIPPED", "DEFERRED", "DECLINED"]
    counts = sorted(tally.items(), key=lambda kv: (order.index(kv[0]) if kv[0] in order else 99))
    newest = sorted(rows, key=lambda r: int(r[0].split("-")[1]), reverse=True)[:8]
    return counts, newest, rows
...
def render() -> str:
    ...
    rows = "".join(
        "<tr><th>%s</th><td>%s</td></tr>" % (inline_md(k), inline_md(v))
        for k, v in build_state_rows(cs)
    )
    role_cards = "".join(
        '<div class="card"><span class="n">%s</span><span class="l">%s</span></div>' % (e(n), e(r))
        for r, n in roles
    )
    items = "".join("<li>%s</li>" % inline_md(i) for i in top_open_items(cs))

    fr_counts, fr_newest, _fr_rows_all = founder_requests(read(FR_INDEX))
    fr_cards = "".join(
        '<div class="card"><span class="n">%s</span><span class="l">%s</span></div>' % (n, e(s))
        for s, n in fr_counts
    )
    fr_rows = "".join(
        "<tr><th>%s <span class=\"tag\">%s</span></th><td>%s</td></tr>"
        % (e(i), e(s), inline_md(subj))
        for i, s, subj in fr_newest
    )
    # Duplicate FR ids have happened before (threads 043/049/053, ADR-048) and are
    # invisible in the index itself, which groups by status rather than by id.
    seen: dict[str, int] = {}
    for fr_id, _s, _subj in _fr_rows_all:
        seen[fr_id] = seen.get(fr_id, 0) + 1
    dupes = sorted(i for i, n in seen.items() if n > 1)
    dupe_warn = (
        '<div class="warn"><strong>Duplicate founder-request ids:</strong> %s. '
        'Two files share each id, so status updates to one are invisible in the other.</div>'
        % e(", ".join(dupes))
    ) if dupes else ""

    fr_block = (
        f'<h2>Founder requests — newest 8 of {sum(n for _, n in fr_counts)}</h2>'
        f'<div class="cards">{fr_cards}</div>'
        f"{dupe_warn}"
        f"<table>{fr_rows}</table>"
    ) if fr_counts else ""
...
<style>
 ...
 .tag {{ font-weight:500; font-size:.7rem; letter-spacing:.05em; color:var(--mut);
         border:1px solid var(--line); border-radius:999px; padding:.05rem .4rem;
         margin-left:.35rem; vertical-align:.05em; }}
...
<div class="meta">Generated {e(gen)} by <code>tools/dashboard.py</code> from
 <code>docs/CURRENT-STATE.md</code>, <code>docs/handoffs/OPEN.md</code> and
 <code>docs/founder-requests/INDEX.md</code>.
