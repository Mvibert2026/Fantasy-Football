...
**For the front-end build, `FRONTEND-SPEC.md` is the one file to hand over.** It restates everything
needed to reproduce the prototype exactly, so it can go to an implementer on its own.
