...
describe('retrieval', () => {
  ...
  it('every context item carries a real, checkable source_path and non-empty text', () => {
    for (const question of ['which draft strategy is best', 'what is the trade deadline in this league']) {
      ...
      for (const item of items) {
        // Two legitimate source shapes, and the point of the assertion is that a
        // reader can go and check the claim -- not that every source is JSON.
        // `docs/assistant-context.md` joined the corpus 2026-07-31 (thread 121);
        // before that the assistant never read it at all. Its items cite a
        // heading anchor rather than a JSON pointer, which is just as checkable.
        expect(item.source_path).toMatch(
          /^(\w+\.json:|docs\/assistant-context\.md#[a-z0-9-]+$)/,
        );
