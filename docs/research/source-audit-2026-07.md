...
| **Fantasy Football Calculator (FFC)** | `[SECONDARY]` Has historical ADP back to 2007 (project record, ADR-018). `[VERIFIED]` No public API docs found; `/api/` is robots-disallowed | `[GAP]` | `[VERIFIED]` **Re-confirmed, unchanged:** `robots.txt` disallows `/api/`, `/ajax/`, `/ajax-v2/`, `/import/`, `/adp/csv/`, `/draft/`, `/rate-my-team/results/`, `/rankings/custom/`. **Nuance the project record misses:** only `/adp/csv/` is disallowed — the HTML ADP pages under `/adp/<format>` are **not** in the disallow list | `[GAP]` **ToS could not be retrieved.** `/terms` → 404; `/terms-of-service` (linked from the footer) rendered navigation only, twice; `/terms-of-service/` → 404. Under the FR-004 conservative default, unverified ToS = treat as blocked. **No FFC data page was fetched** | `[GAP]` — same reason | `[GAP]` | `[SECONDARY]` Overall + positional ADP, by league size and scoring, with sample counts | `[SECONDARY]` History to 2007 (per project record, not re-verified this pass) |
