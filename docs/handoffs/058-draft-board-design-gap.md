...
1. **B1 — the design's ALL-tab tier grouping is NOT the same per-position tier field.** Your
   ...
   **thread 066** to backend asking whether a real `global_tier` field is worth adding — if it lands,
...
2. **C3 — "CURRENT" is not a multi-league marker.** It's the §5.1 sim-staleness state
   ...
   forbids. Opened **thread 067** to backend to ask whether this is coming or correctly deferred.
