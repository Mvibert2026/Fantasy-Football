...
**FROZEN 2026-07-28, existing entries only.** This file no longer accepts new FR entries or status
edits. Unlike `docs/status.md` (a pure append log), FR numbers are referenced by number
throughout the repo (146 references across 40 files at freeze time) and get their `Status:`
mutated over their lifetime by later sessions — that's a concurrent-edit-to-one-blob conflict
shape, not an append shape, so it doesn't fit the same fix as `docs/status.md`. Each FR now gets
its own file instead: **new and future requests live in
[`docs/founder-requests/`](founder-requests/)**, one file per FR (same pattern as
`docs/handoffs/NNN-slug.md`), combined view generated by `tools/founder_requests.py sync` into
[`docs/founder-requests/INDEX.md`](founder-requests/INDEX.md). Protocol:
[`docs/founder-requests/README.md`](founder-requests/README.md). The entries below (FR-001
through the last one recorded here) are the historical archive, unmodified — do not add to them
or edit their `Status:` lines going forward; open a new FR or amend in the FR's own file instead.

**Every agent, in every surface, must capture founder requests.** If the founder says anything in
a Claude Code session, in the Cowork chat, or to Design that expresses a want, a constraint, a
preference, or a "wouldn't it be good if" — it gets an entry before that session ends. Do not
judge whether it is important enough. Do not wait for it to be formally specced.
