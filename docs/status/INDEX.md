...
`npx tsc -b --noEmit` clean throughout. Full suite: 277 passing pre-session baseline (30 files) →
280 passing (30 files) after this session's 3 new `draft.test.ts` cases and 2 updated existing
assertions (`opponents.test.tsx`, `draft-room-middle-pane-tabs.test.tsx`), verified green after
