...
**Why it blocks:** (B) is required to compute the FRAGILE veto's null distribution of `Δ̄_LOOmin` —
without it the veto cannot be evaluated at any cost. (A)'s lower tail is required before **any**
HARM cell can be assessed: C1 published the upper tail only, which leaves C1's F1-TE HARM (−0.0285)
and **B1's WR HARM of −0.0125, on which arms G1 and G1a were rejected**, ungradeable against their
own null.

**No decision rule attached** — this is publication of data you already generated, not a test.
...
`h = 20`, `L = ceil(2M/q) − 1 = 2,599`, use 3,000), the §4.4 verdict taxonomy including the FRAGILE
veto and the retained VOID rule, the §4.6 reporting fields, and the §4.7 derived snap tolerance
...
committed. It is not executable until M-1 (which blocks the FRAGILE veto and every HARM cell),
