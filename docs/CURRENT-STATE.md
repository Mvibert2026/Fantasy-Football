...
| | Value | Notes |
|---|---|---|
| Backend branch / commit | `worktree-agent-afa13ac8a8bd0c533`, `75bf095b48567de06edd67b6bb8a8ea5635814cc` | `git rev-parse --abbrev-ref HEAD` / `HEAD` |
...
are both the wrong shape — ADP not expert consensus; and FFC, though now unblocked, publishes
ADP rather than expert consensus too, so it does not answer this need either).
...
**The board is 510 players as of 2026-07-29** (`data/export/board.json`, measured from the file;
the figure read 511 until the claim checker compared it to disk). The coverage ratio has *not*
been re-measured against the larger universe — do not read
...
**Correction, 2026-07-29:** the Predictions tab was wrongly listed in this section until today.
It ships — `frontend/ui/views/Predictions.tsx` exists and is routed from both
`frontend/ui/App.tsx:167` and `frontend/ui/StandaloneApp.tsx:127`. Registered as a checked claim
in `docs/state-claims.toml` so the line cannot come back. (Rendering quality is a separate
question and still needs a screenshot; existence is settled.)
