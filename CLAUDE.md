...
| `docs/launch-prompts.md` | The short prompt to paste to start a chat session as a role, per role. The founder's PM launch prompt lived only in chat and was lost by 2026-08-07; this is where it lives now. Keep the prompts short — `CLAUDE.md` already carries the law, so a launch prompt only names the role and starts the work |
| `docs/rented-box-runbook.md` | How to run the ADR-070 campaign on a rented CPU box **without a terminal** — the provider's env-var and container-start-command fields are the whole setup. Also: when a box is worth renting at all (rarely — Actions is free and uncapped on a public repo), and that **Stop does not end the bill, Terminate does** |
